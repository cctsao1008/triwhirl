...
inline constexpr std::uint8_t kStandupTraceVersion = 1U;
...
static_assert(sizeof(StandupTraceRecord) == 24U,
              "standup trace record must remain 24 bytes");
static_assert(sizeof(StandupTraceFrameHeader) == 28U,
              "standup trace frame header must remain 28 bytes");
