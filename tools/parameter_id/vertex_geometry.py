#!/usr/bin/env python3
"""Shared three-vertex geometry helpers for TriWhirl identification.

The Reuleaux body has three legitimate upright vertex equilibria separated by
120 body degrees.  ``vertex_a_deg`` is only a naming/reference anchor in the
current IMU frame; B and C are generated geometrically from it.  The helpers
use circular angle distance so wraparound at +/-180 degrees is harmless.
"""
...
def classify_vertex_deg(
    ...
    """Classify a held upright angle as A/B/C, or None if it is between vertices."""
